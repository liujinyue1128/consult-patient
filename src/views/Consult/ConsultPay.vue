...
const loadData = async () => {
  try {
    console.log(store.consult.illnessType)
...
const submit = async () => {
  ...
  loading.value = true
  const res = await createConsultOrder({
    consultFlag: 1,
    illnessDesc: '睡眠不足',
    illnessType: 1,
    couponId: null,
    illnessTime: 1,
    patientId: '7020955212587008',
    pictures: [
      {
        id: '7020946657820672',
        url: 'http://yjy-teach-oss.oss-cn-beijing.aliyuncs.com/consult/production/20240426/7020946656829440.jpg'
      }
    ],
    type: 2
  })
...
const onClose = () => {
  return showConfirmDialog({
    title: '关闭支付',
    message: '取消支付后将无法获得医生回复，医生接诊名额有限，是否确认关闭？',
...
const pay = async () => {
  ...
  const res = await getConsultOrderPayUrl({
    ...
    payCallback: 'http://localhost:5173/roomStatus'
  })
